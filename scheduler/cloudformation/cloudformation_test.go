...
func TestScheduler_Run_Attached(t *testing.T) {
	...
	e.On("WaitUntilTasksRunning", &ecs.DescribeTasksInput{
		Cluster: aws.String("arn:aws:ecs:us-east-1:012345678910:cluster/cluster"),
	...
	d.On("ListContainers", docker.ListContainersOptions{
		Filters: map[string][]string{
			"label": []string{
				fmt.Sprintf("com.amazonaws.ecs.task-arn", "arn:aws:ecs:us-east-1:012345678910:task/fdf2c302-468c-4e55-b884-5331d816e7fb"),
			},
		},
	...
	d.On("StartExec", "1234", docker.StartExecOptions{
		Detach:       false,
		Tty:          true,
		InputStream:  stdin,
		OutputStream: stdout,
...
func (m *mockECSClient) WaitUntilTasksRunning(input *ecs.DescribeTasksInput) error {
	args := m.Called(input)
	return args.Error(0)
}
...
func (m *mockDockerClient) CreateExec(options docker.CreateExecOptions) (*docker.Exec, error) {
	args := m.Called(options)
	return args.Get(0).(*docker.Exec), args.Error(1)
}

func (m *mockDockerClient) StartExec(execID string, options docker.StartExecOptions) error {
	args := m.Called(execID, options)
	return args.Error(0)
}
