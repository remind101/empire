...
//
//	589611bf-4e6c-4fc2-a77a-d61dca412289.v1.web.1
type Name string

// NewName returns a new Name with the proper format.
func NewName(id apps.ID, v releases.Version, pt processes.Type, i int) Name {
...
func buildJobs(appID apps.ID, version releases.Version, image images.Image, vars configs.Vars, formation []*formations.CommandFormation) []*Job {
	var jobs []*Job
	...
	for _, f := range formation {
		...
		for i := 1; i <= f.Count; i++ {
			j := &Job{
				Name:        NewName(appID, version, f.ProcessType, i),
				Environment: env,
				Execute: Execute{
					Command: cmd,
					Image:   image,
