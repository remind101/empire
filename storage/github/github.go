...
const (
	FileManifest = "app.json"
	FileEnv      = "app.env"
	FileImage    = "image.txt"
	FileServices = "services.json"
)

// Manifest describes basic information about the application.
...
func (s *Storage) Releases(q empire.ReleasesQuery) ([]*empire.Release, error) {
	...
	commits, _, err := s.github.Repositories.ListCommits(s.Owner, s.Repo, &github.CommitsListOptions{
		SHA:  s.Ref,
		Path: s.Path(app.Name, FileManifest),
	})
	if err != nil {
		return nil, err
	}
...
func (s *Storage) treeEntries(app *empire.App) ([]github.TreeEntry, error) {
	manifestFile, err := jsonMarshal(&Manifest{
		ID:      app.ID,
		Release: app.Version,
	})
	if err != nil {
	...
	entries := []github.TreeEntry{
		{
			Path:    github.String(s.Path(app.Name, FileManifest)),
			Type:    github.String("blob"),
			Mode:    github.String(BlobPerms),
			Content: github.String(string(manifestFile)),
		},
	}
...
func loadApp(f contentFetcher, app *empire.App) (*empire.App, error) {
	var m Manifest
	if err := decodeFile(f, PathJoin(app.Name, FileManifest), &m); err != nil {
		return nil, err
	}
